...
Organize metadata for image submission to IDR.
...
```{r}
outpath <- "idr0000-way-lincscellpainting"
...
```{r}
# template to fill in
template_lib <- fread("idr0000-lastname-example-master/screenA/idr0000-screenA-library.txt")
```
...
# same from cp
sample_meta_cp <- fread("https://github.com/cmap/lincs-cell-painting/blob/master/profiles/cell_count/2016_04_01_a549_48hr_batch1_metadata_cell_count_summary.tsv.gz")
cp_plates <- scan(file="lincs_plates_final.txt", what="char", sep="\n")
```

```{r}
# pert meta (for pert_iname, etc) from repurposing hub
repo_drugs <- fread("repurposing_drugs_20200324.txt", skip=9)
repo_samples <- fread("repurposing_samples_20200324.txt", skip=9)
```
...
# plate_map[ncp < 50]
```
None of these are A549 so probably don't need to worry about them.
...
fwrite(out_lib, file.path(opath, "idr0000-screenA-library.txt"), sep="\t")
```
